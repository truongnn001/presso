...
class PDFConverterGUI:
    ...
    def __init__(self, root):
        ...
        # File config để lưu đường dẫn (tìm trong pdf2markdown/config/ hoặc root)
        config_dir = Path(__file__).parent / "pdf2markdown" / "config"
        if not config_dir.exists():
            config_dir = Path(__file__).parent  # Fallback về root
